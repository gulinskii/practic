...
if result <= 300:
    ...
    price_rounded = Decimal(str(price)).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP) #для более точного округления
    price_double = price / result

    print("Предыдущее: {0}\nТекущее: {1}\nИспользовано: {2}\nК оплате: {3}\nСр. цена m^3: {4:.2f}"
    .format(wased, new, result, price_rounded, price_double))
    ...
    price_rounded = Decimal(str(price)).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
    price_double = price / result
